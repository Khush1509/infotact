...
class AnalyticsConsumer:
    ...
    def __init__(
        ...
        self.topic = topic
        logger.info(
            f"Initializing consumer for topic '{topic}' on server '{bootstrap_servers}'"
        )
    ...
    def run(self):
        ...
        #    Consumer({'bootstrap.servers': ..., 'group.id': 'analytics-group'})
    ...
    def aggregate_event(self, event_data):
        ...
        logger.info(
            f"Aggregating event: {event_type} for video: {video_id} at {timestamp}"
        )

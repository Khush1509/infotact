...
def upload_chunk():
    ...
    file_path = os.path.join(current_app.config['UPLOAD_FOLDER'], f"{video_id}_chunk_{chunk_index}")
    file.save(file_path)
...
def track_analytics():
    ...
    if not data or 'video_id' not in data or 'event' not in data or 'timestamp' not in data:
